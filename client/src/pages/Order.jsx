...
function Order() {
  ...
  const [addFoodOption, setAddFoodOption] = useState(null);
  ...
  const fetchData = async () => {
    try {
      ...
      if (billData.bill) {
        ...
        setParcel(billData.bill.parcel === 1);
      }
      ...
      const loadedOrders = (billData.orders || []).map((order) => ({
        ...
        selectedOption: order.option_name || "",
  ...
  const openFoodOptions = (food) => {
    ...
    const options = foodOptions.filter(
      (option) => option.category_id === food.category_id,
    );
  ...
  const addFoodWithType = (addFoodOption) => {
    ...
    if (currentOptions.length > 0 && !addFoodOption) {
    ...
    if (!addFoodOption) {
      const finalPrice = foodPrice(selectedFood) * quantity;

      const updatedFood = {
        ...selectedFood,

        selectedOption: addFoodOption ? addFoodOption.option_name : "",

        option_id: addFoodOption ? addFoodOption.option_id : null,
      ...
      setBillItems([...billItems, updatedFood]);
    ...
    const finalPrice =
      (foodPrice(selectedFood) + addFoodOption.extra_price) * quantity;
  ...
  const sendKOT = async () => {
    try {
      const newItems = billItems.filter((item) => item.isNew);
    ...
    <div className="orders">
      ...
      <div className="main-layout">
        ...
        <div className="bill-section">
          <h2 className="bill-title">TOTAL BILL</h2>
          ...
          <div className="bill-top">
            <div className="bill-info">
              ...
              <div className="parcel-box">
                ...
                <select
                  ...
                  onChange={(e) => setParcel(e.target.value === "true")}
                >
            ...
            <div className="bill-name-transfer">
              ...
              <select
                className="transfer-select"
                value={transferTable}
                onChange={(e) => setTransferTable(e.target.value)}
              >
          ...
          <div className="bill-buttons">
            <button onClick={sendKOT}>KOT</button>

            <button>KOT & PRINT</button>
            <button>BILL & PRINT</button>

            <button onClick={() => setShowConfirm(true)}>BILL</button>
          </div>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="confirm-modal">
            ...
            <p>
              Are you sure you want to generate the bill? This will close the
              table and mark it as available.
            </p>
